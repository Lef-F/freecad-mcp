...
- **Selected objects appear light blue**: FreeCAD highlights selected objects with a blue overlay in screenshots. This is NOT a color problem — call `FreeCADGui.Selection.clearSelection()` before checking colors, or just recognize the highlight.
- **`Part.fuse()` crash risk**: Fusing many complex lofted shapes can freeze/crash FreeCAD. Use `Part.makeCompound()` for display grouping or individual `Part::Feature` objects in a group instead.
- **App::Part Placement composes for children (Shape stays in local coords)**: assigning a Shape built in world coords to a `Part::Feature` and then calling `parent.addObject(feature)` where `parent` is an App::Part with non-zero Placement causes the rendered object to float by the parent's offset. Either keep top-level or transform Shape to parent-local first. See `.claude/context/freecad-origins.md` "Writing a Shape inside an App::Part".
- **OCCT Booleans return `Compound`, not `Solid`**: `shape.cut/common/fuse` results need `.Solids[0]` extraction before assignment to `Part::Feature.Shape` if you want `ShapeType == "Solid"`. See `.claude/context/freecad-modeling-guide.md` "Boolean Operations and Cutter Selection".
- **PartDesign Pocket with 3D curved-face Profile creates phantom `SurfaceOfExtrusion` faces**: Pocket profiles must be 2D sketches. Binding curved 3D faces (terrain shells, BSplineSurfaces) as Profile leaves twisted leftover faces on the body Tip. Use Part workbench Booleans (`Part.cut`/`Part.common`) against valid solids instead. See `.claude/context/freecad-modeling-guide.md`.
- **GUI task timeout is not a recompute failure**: the 30s RPC response timeout fires on slow `doc.recompute()` calls (heavy TechDraw, large Booleans), but the operation usually completes. Re-query state with a lighter call before retrying; blind retries can leave duplicate objects. See `.claude/context/known-issues.md`.
...
```bash
# macOS — FreeCAD 1.1+ uses versioned directories:
cp -r addon/FreeCADMCP ~/Library/Application\ Support/FreeCAD/v1-1/Mod/
# macOS — FreeCAD 1.0.x (unversioned):
...
> **Note:** FreeCAD 1.1+ uses versioned user data directories (e.g., `v1-1/`). Check
> `FreeCAD.getUserAppDataDir()` in the FreeCAD Python console to find the correct path.

Verify the copy is in sync (only `__pycache__` should differ):
```bash
diff -r addon/FreeCADMCP ~/Library/Application\ Support/FreeCAD/v1-1/Mod/FreeCADMCP
```
